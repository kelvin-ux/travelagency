...
CREATE OR REPLACE TYPE OcenaHoteli_typ AS OBJECT (
    ocena_id         NUMBER,
    ref_uzytkownik   REF Uzytkownik_typ,  -- np. do tabeli Uzytkownicy_tab
    ocena            Ocena_typ,           -- nowa definicja z data
    opinia           VARCHAR2(1000),
    data_oceny       DATE
);
/

-- Atrakcja_typ (z referencją do oceny – jeśli potrzebne)
...
-- Adres_typ (podstawowa definicja)
...
-- Adresy_typ (opakowanie: ID + Adres)
...
-- Kategorie_typ z polami typu BOOLEAN
...
-- OfertyWakacyjne_typ: dodajemy "duration"
CREATE OR REPLACE TYPE OfertyWakacyjne_typ AS OBJECT (
    ...
    duration        NUMBER(5,2)    -- liczba dni (przykładowo w formacie x.xx)
);
/

-- Promotions_typ (z referencją do oferty)
